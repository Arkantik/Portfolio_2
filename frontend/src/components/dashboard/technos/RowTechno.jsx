...
// Components
import DropdownTechno from "./DropdownTechno";

// Helpers
import { capitalizeText } from "../../../helpers/getName";

// Services
import { deleteTechno } from "../../../services/techno.service";

export default function RowTechno({ techno, setFlagTechnos }) {
  ...
  const handleDeleteTechno = (id) => {
    deleteTechno(id)
      .then((res) => {
        if (res?.status === 204)
          toast.success("Techno successfully deleted!", TOAST_DEFAULT_CONFIG);
  ...
  return (
    <>
      <tr className="dark:border-neutral w-full justify-between border-b">
        <td className="px-4 py-3 text-sm">{techno.id}</td>
        <td className="px-4 py-3 text-sm">{capitalizeText(techno.name)}</td>
        <td className="flex items-center gap-5 px-4 py-3 text-sm">
          <img
            src={`${import.meta.env.VITE_BACKEND_URL}/uploads/images/technos/${
              techno.img
            }`}
            alt={techno.name}
        ...
        <td className="px-4 py-3 text-sm">
          <span className="flex h-full w-full gap-4 ">
            ...
            <button
              type="button"
              className="group"
              onClick={() => handleDeleteTechno(techno.id)}
            >
              <svg
                width="18"
                height="18"
